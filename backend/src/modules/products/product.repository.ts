...
// Finds up to 6 products in the same category as the source product.
// Strategy:
//  1. Try ±70% price band first (wide enough for realistic small datasets)
//  2. If fewer than 2 results, fall back to all products in same category
//  3. Always exclude source product itself
export const findSimilarProducts = async (
  productId: string,
  categoryId: string,
  discountPrice: number
) => {
  // Price band: ±70% of the current product's discountPrice
  const lower = new Prisma.Decimal(discountPrice * 0.3);
  const upper = new Prisma.Decimal(discountPrice * 1.7);

  const priceRangeResults = await db.product.findMany({
    where: {
      isActive: true,
      categoryId,
      id: { not: productId },
      discountPrice: { gte: lower, lte: upper },
    },
    select: productSelect,
    orderBy: [
      { rating: "desc" },
  ...
  // If we got at least 2 relevant results, return them
  if (priceRangeResults.length >= 2) {
    return priceRangeResults;
  }

  // Fallback: return any other active product in the same category
  // This ensures similar products always works even with small datasets
